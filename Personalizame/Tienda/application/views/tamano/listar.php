...
<div class="card">
	<div class="card-header">
		<h2>
			Listado de tamaños<small>Introduce el filtro que desees para una búsqueda más precisa.</small>
		</h2>
	</div>
	...
	<div id="data-table-command-header"
		...
		<form class="row" id="idFormFiltro" role="form"
			action="<?= base_url() ?>tamano/listarPost" method="post">
			<div class="col-sm-3">
			...
			<button class="btn btn-primary" onclick="function f() {document.getElementById('idFormFiltro').submit();}"><span class="glyphicon glyphicon-search"></span> Filtrar</button>
				<!-- 
				<button
					onclick="function f() {document.getElementById('idFormFiltro').submit();}"
					class="btn btn-info">
	...
	<table id="data-table-command"
		...
		<tbody>
				...
				<td class="text-left">
					<form id="idFormedit" action="<?=base_url();?>tamano/modificarPost"
						method="post">
					...
					<form id="idFormedit" action="<?=base_url();?>tamano/borrarPost"
						method="post">
						<input type="hidden" name="idTamanos[]" value="<?= $tamano -> id?>">
						<button
							class="btn btn-icon command-delete waves-effect waves-circle"
							data-row-id="<?= $tamano['id'] ?>">
